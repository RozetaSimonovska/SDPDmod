                   ...
                   mny,mnx,mty,mtx,effect,tind,sind,Wy0)
{
  ...
  int.se<-sqrt(sige/(n0*t0)+
                 sige*t(colMeans(X0))%*%solve(
                   t(X)%*%X)%*%(colMeans(X0))) ## s.e. itercept
  ...
  if(effect=="individual"){
    sfe<-mny-mnWy*rho-c(mnx%*%beta)-
      rep(intercept,n0) ## spatial fixed effects coefficients
    xhat<-X0%*%beta+rep(sfe,t0)+rep(intercept,n0*t0)
    sfe.se<-sqrt(sige/t0*rep(1,n0)+diag(
      sige*mnx%*%solve(t(X)%*%X)%*%t(mnx))) ###standard errors
    ...
    tfe.se<-sqrt(sige/n0*rep(1,t0)+diag(
      sige*mtx%*%solve(t(X)%*%X)%*%t(mtx)))
    tfe.t<-tfe/tfe.se
