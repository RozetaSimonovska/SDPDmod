...
summary.SDPDm <- function(object,...) {
  if(class(object)=="SDPDm"){
    if(object$dynamic){
      result<-cat(paste0(object$model," dynamic panel model with ",object$effects, " fixed effects\n"))
    }else{
      result<-cat(paste0(object$model," panel model with ",object$effects, " fixed effects\n"))
    }
    result<-cat("\nCall:\n")
